...
if (empty($chatUser)) {
    ...
    // Fetch chat history between the two users
    $stmt = $mysqli->prepare("SELECT * FROM chats WHERE ((from_user = ? AND to_user = ? AND deleted_by_sender = FALSE) OR (from_user = ? AND to_user = ? AND deleted_by_receiver = FALSE)) ORDER BY time_sent ASC");
...
<head>
    ...
    margin-top: 10px;
        ...
        .message-form input[type="text"] {
           ...
           // margin-right: 10px;
           //width: 70%;
           width: calc(70% - 10px); /* Subtracting padding from the total width */
        }
        .message-form input[type="submit"] {
            ...
            cursor: pointer;
            width: calc(25% - 3px); /* Subtracting padding from the total width */
        }
...
<div class="chat-header">
        ...
        <h2><?php echo $chatUser['first_name']; ?> <?php echo $chatUser['last_name']; ?></h2>
    </div>
...
function deleteMessage(messageId) {
    // Send an AJAX request to mark the message as deleted
    const xhr = new XMLHttpRequest();
    xhr.open('POST', 'deleted_by.php', true);
    xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
            // Hide the message from the chat history
